...
fn gen_arrow_func_expr<'a>(node: &'a ArrowExpr, context: &mut Context<'a>) -> PrintItems {
  ...
  fn gen_arrow_body<'a>(node: &'a ArrowExpr, header_start_lsil: LineStartIndentLevel, context: &mut Context<'a>) -> PrintItems {
    ...
    if open_brace_token.is_some() {
      ...
      items.push_condition(conditions::indent_if_start_of_line(generated_body.into(), context.config.hanging_indent_times));
  ...
  if node.op() == AssignOp::Assign {
    if let Expr::Assign(right) = node.right {
      if right.op() == AssignOp::Assign {
        ...
        let multi_line_indent_times = context.config.multi_line_indent_times;
...
fn gen_conditional_expr<'a>(node: &'a CondExpr, context: &mut Context<'a>) -> PrintItems {
  ...
  items.extend(ir_helpers::new_line_group(with_queued_indent_times({
  ...
  let cons_and_alt_items = {
    ...
    items.push_condition({
      ...
      indent_times_if_sol_and_same_indent_as_top_most(ir_helpers::new_line_group(items), top_most_il, hanging_indent_times)
    });
  ...
  if top_most_data.is_top_most {
    items.push_condition(conditions::indent_if_start_of_line(cons_and_alt_items, hanging_indent_times));
  } else {
    items.push_condition(indent_times_if_sol_and_same_indent_as_top_most(cons_and_alt_items, top_most_data.il, hanging_indent_times));
  }
    ...
    ConditionalTokenComments {
      previous_lines: {
        ...
        if items.is_empty() {
          ...
          new_items.push_condition(indent_times_if_sol_and_same_indent_as_top_most(items, top_most_il, hanging_indent_times));
          new_items
        }
      },
...
// TODO: add indent_times here
fn indent_times_if_sol_and_same_indent_as_top_most(items: PrintItems, indent_level: IndentLevel, indent_times: u8) -> Condition {
  let items = items.into_rc_path();
  if_true_or(
    ...
    with_indent_times(items.into(), indent_times),
    items.into(),
  )
}
    ...
    if let Node::TplElement(node_tpl) = node {
      ...
      if !leading_comment_items.is_empty() || !trailing_comment_items.is_empty() {
        ...
        items.extend(if keep_on_one_line {
          ...
          ir_helpers::surround_with_newlines_indented_if_multi_line(new_line_group(generated_expr), context.config.indent_width, context.config.hanging_indent_times)
        } else {
          generated_expr
        });
      }
...
fn gen_jsx_opening_element<'a>(node: &'a JSXOpeningElement, context: &mut Context<'a>) -> PrintItems {
  ...
  if node.attrs.len() == 1 && node.type_args.is_none() && is_jsx_attr_with_string(&node.attrs[0]) {
    ...
    let mut multi_line_options = ir_helpers::MultiLineOptions::surround_newlines_indented(hanging_indent_times, multi_line_indent_times);
    ...
    |context| {
      ir_helpers::gen_separated_values(
        ...
        ir_helpers::GenSeparatedValuesOptions {
          ...
          multi_line_options: ir_helpers::MultiLineOptions::same_line_no_indent(context.config.hanging_indent_times, context.config.multi_line_indent_times),
          force_possible_newline_at_start: false,
        },
      )
      .items
...
fn gen_conditional_type<'a>(node: &'a TsConditionalType, context: &mut Context<'a>) -> PrintItems {
  ...
  if is_parent_conditional_type {
    ...
    items.push_condition(conditions::indent_if_start_of_line(false_type_generated, hanging_indent_times));
  }
  ...
  items.extend(gen_surrounded_by_tokens(
    |context| {
      ...
      if force_use_new_lines {
        ...
        ir_helpers::surround_with_newlines_indented_if_multi_line(inner_items, context.config.indent_width, context.config.hanging_indent_times)
      }
    },
    |_| None,
...
fn gen_type_parameters<'a>(node: TypeParamNode<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  items.extend(gen_separated_values(
    GenSeparatedValuesParams {
      ...
      single_line_space_at_end: false,
      custom_single_line_separator: None,
      multi_line_options: ir_helpers::MultiLineOptions::surround_newlines_indented(context.config.hanging_indent_times, context.config.multi_line_indent_times),
      force_possible_newline_at_start: false,
      node_sorter: None,
    },
    context,
...
fn gen_union_or_intersection_type<'a>(node: UnionOrIntersectionType<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  let multi_line_options = if !is_parent_union_or_intersection {
    if use_surround_newlines(node.node, context) {
      ir_helpers::MultiLineOptions::surround_newlines_indented(hanging_indent_times, multi_line_indent_times)
    } else {
    ...
    |context| {
      gen_separated_values(
        GenSeparatedValuesParams {
          ...
          custom_single_line_separator: None,
          multi_line_options: ir_helpers::MultiLineOptions::surround_newlines_indented(hanging_indent_times, multi_line_indent_times),
          force_possible_newline_at_start: false,
          node_sorter: None,
        },
        context,
    ...
    |context| {
      ...
      items.extend(gen_separated_values(
        GenSeparatedValuesParams {
          ...
          single_line_space_at_end: space_around,
          custom_single_line_separator: None,
          multi_line_options: MultiLineOptions::surround_newlines_indented(hanging_indent_times, multi_line_indent_times),
...
fn gen_node_in_parens<'a>(gen_node: impl FnOnce(&mut Context<'a>) -> PrintItems, opts: GenNodeInParensOptions, context: &mut Context<'a>) -> PrintItems {
  ...
  return gen_surrounded_by_tokens(
    |context| {
      ...
      if force_use_new_lines {
        surround_with_new_lines(with_indent_times(generated_node, hanging_indent_times))
      } else if opts.prefer_hanging {
        generated_node
      } else {
        ir_helpers::surround_with_newlines_indented_if_multi_line(generated_node, context.config.indent_width, hanging_indent_times)
      }
    },
    |_| None,
    GenSurroundedByTokensOptions {
  ...
  items.push_condition(conditions::indent_if_start_of_line(ir_helpers::new_line_group({
    ...
    items.extend(gen_separated_values(
      GenSeparatedValuesParams {
        ...
        single_line_space_at_end: false,
        custom_single_line_separator: None,
        multi_line_options: ir_helpers::MultiLineOptions::new_line_start(hanging_indent_times, multi_line_indent_times),
        force_possible_newline_at_start: false,
        node_sorter: None,
      },
      context,
...
fn gen_object_like_node<'a>(opts: GenObjectLikeNodeOptions<'a>, context: &mut Context<'a>) -> PrintItems {
  let mut items = PrintItems::new();
  let multi_line_indent_times = context.config.object_like_indent_times;
  let hanging_indent_times = context.config.hanging_indent_times;
...
fn gen_for_flattened_member_like_expr<'a>(node: FlattenedMemberLikeExpr<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  for (i, item) in node.nodes.iter().enumerate().skip(1) {
    ...
    if item.is_computed() {
      items.push_condition(indent_if_start_of_line_or_start_of_line_indented(generated_item, 1));
    } else {
      items.push_condition(conditions::indent_if_start_of_line(generated_item, 1));
    }
  }
  ...
  return new_line_group(gen_surrounded_by_tokens(
    |context| {
      if force_use_new_lines {
        ...
        ir_helpers::surround_with_newlines_indented_if_multi_line(gen_inner(context), context.config.indent_width, context.config.hanging_indent_times)
      }
    },
    |_| None,
    GenSurroundedByTokensOptions {
...
fn gen_decorators<'a>(decorators: &[&'a Decorator<'a>], is_inline: bool, context: &mut Context<'a>) -> PrintItems {
  ...
  let separated_values_result = gen_separated_values_with_result(
    GenSeparatedValuesParams {
      ...
      custom_single_line_separator: None,
      multi_line_options: ir_helpers::MultiLineOptions::same_line_no_indent(0, 0),
      force_possible_newline_at_start: false,
      node_sorter: None,
    },
    context,
...
fn gen_assignment_like_with_token<'a>(expr: Node<'a>, op: &'static str, op_token: Option<&TokenAndSpan>, context: &mut Context<'a>) -> PrintItems {
  ...
  let generated_assignment = {
    ...
    let assignment = if had_op_trailing_comments {
      ...
      conditions::indent_if_start_of_line(assignment, context.config.hanging_indent_times).into()
    };
